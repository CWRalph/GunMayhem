...
void jar_xm_generate_samples_16bit(jar_xm_context_t* ctx, short* output, size_t numsamples) {
    ...
    if(output){
        for(int x=0;x<2*numsamples;x++) output[x] = (short)(musicBuffer[x] * 32767.0f); // scale sample to signed small int
    }
...
void jar_xm_generate_samples_8bit(jar_xm_context_t* ctx, char* output, size_t numsamples) {
    ...
    if(output){
        for(int x=0;x<2*numsamples;x++) output[x] = (short)(musicBuffer[x] * 127.0f); // scale sample to signed 8 bit
    }
    JARXM_FREE(musicBuffer);
}
